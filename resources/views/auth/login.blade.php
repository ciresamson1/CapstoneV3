        ...
        <div class="relative hidden flex-col justify-between bg-slate-950 px-10 py-12 text-white lg:flex lg:w-[46%]">
            <div>
                ...
                <h1 class="mt-14 text-4xl font-bold leading-tight">
                    Welcome back.<br>
                    <span class="text-violet-400">Let's get to work.</span>
                </h1>
        ...
        <div class="flex flex-1 flex-col items-center justify-center px-8 py-12">
            <div class="w-full max-w-md">
                ...
                <div class="mb-8 flex items-center gap-3 lg:hidden">
                    <div class="flex h-10 w-10 items-center justify-center rounded-2xl bg-violet-500 text-sm font-bold text-white">P</div>
                    <span class="text-sm font-semibold tracking-wide text-slate-500">PCMS Portal</span>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}"
                            class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
                    ...
                    <div>
                        <div class="mb-2 flex items-center justify-between">
                            ...
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-xs text-violet-500 hover:underline">Forgot password?</a>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-2 pt-1">
                        <input type="checkbox" name="remember" id="remember" class="h-4 w-4 rounded border-slate-300 accent-violet-500">
                        <label for="remember" class="text-sm text-slate-600">Remember me</label>
                    </div>

                    <button type="submit"
                        class="w-full rounded-2xl bg-violet-500 px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-violet-200 transition hover:bg-violet-600">
                        Sign in
                    </button>
                </form>
                ...
                @if (Route::has('register'))
                    <p class="mt-6 text-center text-sm text-slate-500">
                        ...
                        <a href="{{ route('register') }}" class="font-semibold text-violet-500 hover:underline">Register</a>
                    </p>
                @endif
